  ...
  if (*p == MAGIC) {
    if (is_interleaved) {
      for (total = 0; total < size_payload; total += (l_inter + 4)) {
        ...
        p += l_inter;
        if (l_inter != 0)
          printf("RTSP Interleaved Frame found - RTP packet saved in buffer "
                 "(length = %d)\n",
                 l_inter);
      }
    } else
      return -2; // ERROR - found MAGIC but not interleaved in SETUP
  } else {
    ...
    **/
    /* { */
    js_ret += snprintf((json_buffer + js_ret), buffer_len,
                       "{ \"rtsp_report_information\":{ ");
    // TYPE
    if (msg->msg_type == 0)
    ...
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"seq-num\":%s, ",
                       (msg->seq_num == NULL) ? "-" : msg->seq_num);
    // SESSION
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"session\":%s, ",
                       (msg->session == NULL) ? "-" : msg->session);
    // SERVER
    js_ret +=
        snprintf((json_buffer + js_ret), buffer_len - js_ret, "\"server\":%s, ",
                 (msg->server == NULL) ? "-" : msg->server);
    // URI
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"uri\":%s, ", (msg->uri == NULL) ? "-" : msg->uri);
    // UA
    js_ret +=
        snprintf((json_buffer + js_ret), buffer_len - js_ret,
                 "\"user-agent\":%s, ", (msg->ua == NULL) ? "-" : msg->ua);
    // CONTENT BASE
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"content-base\":%s, ",
                       (msg->content_base == NULL) ? "-" : msg->content_base);
    // CONTENT LEN
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"content-len\":%s, ",
                       (msg->content_len == NULL) ? "-" : msg->content_len);
    // CONTENT TYPE
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"content-type\":%s, ",
                       (msg->content_type == NULL) ? "-" : msg->content_type);
    // PROTOCOL
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"protocol\":%s, ",
                       (msg->protocol == NULL) ? "-" : msg->protocol);
    // CACHE CONTROL
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"cache control\":%s, ",
                       (msg->cache_control == NULL) ? "-" : msg->cache_control);
    // TRANSPORT
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"transport\":%s ",
                       (msg->transport == NULL) ? "-" : msg->transport);
    // SDP
    js_ret += snprintf((json_buffer + js_ret), buffer_len - js_ret,
                       "\"sdp\":%s ", (msg->sdp == NULL) ? "-" : msg->sdp);
    /* } */
    js_ret +=
        snprintf((json_buffer + js_ret - 1), (buffer_len - js_ret + 1), " }");
    js_ret +=
        snprintf((json_buffer + js_ret - 1), (buffer_len - js_ret + 1), " }");
...
int rtsp_message_parser(rtsp_message *msg, char *rtspMessage, int length) {
  ...
  **/
  ...
  /**
     ...
     -- if token is a Method definition the msg is a request (value must be
  saved)
  **/
  // Get the token
  ...
  **/
  ...
  /**
     ...
     - rest of pkt must be parsed to extract header field, which give us much
  more details
  ...
  **/
    ...
    if (token != NULL) {
      ...
      **/
      ...
      // CONTENT-TYPE
        ...
        token = strtok(NULL, delim);
        content_type = token;
        if (!strncmp(content_type, "application/sdp",
                     strlen("application/sdp")))
          is_sdp = 1;
      }
      // CONTENT-LENGTH
      ...
      */
      ...
      // Check if we've hit the end of the message. The first \r is missing
      // because it's been tokenized
  ...
  create_rtsp_message(msg, protocol, uri, command, seq_num, status_code, server,
                      ua, cache_control, content_len, content_type,
                      content_base, last_mod, range, session, transport, flag,
                      sdp);
