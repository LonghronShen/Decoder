...
**/
int diameter_parser(const unsigned char *packet, int size_payload,
                    char *json_buffer, int buffer_len) {
  char type[20] = {0};
  ...
  /* TYPE => check if Flag bit R is set to 0-1-2-3
   * (Answer-Request-Request/Proxyable-Proxyable) */
  ...
  /* COMMAND */
  command = diameter->com_code[2] + (diameter->com_code[1] << 8) +
            (diameter->com_code[0] << 8);
  ...
  /*** CREATE JSON BUFFER ***/
  js_ret += snprintf(json_buffer, buffer_len, DIAMETER_HEADER_JSON, type,
                     command, app_id, hop_by_hop_str, end_to_end_str);

  /***** END of parsing Diamter Header *****/

  // Calculate the length of payload from header field
  u_int16_t length = diameter->length[2] + (diameter->length[1] << 8) +
                     (diameter->length[0] << 8);
  ...
  **/
  ...
  while (start < end && offset < length) {
    ...
    // check AVP flag - search the presence of Vendor-ID field (4 bytes
    // optional)
    if (CHECK_BIT(avp->flag, 8) == 128) {
      /* vendor_id = start[3] + (start[2] << 8) + (start[1] << 8) + (start[0] <<
       * 8); */
    ...
    case AUTH_SESS_ST: {
      ...
      int auth_val =
          start[3] + (start[2] << 8) + (start[1] << 8) + (start[0] << 8);
      if (auth_val == 1)
        snprintf(auth_sess_st, strlen("NO-STATE-MAINTAINED") + 1,
                 "NO-STATE-MAINTAINED");
      else if (auth_val == 0)
        snprintf(auth_sess_st, strlen("STATE-MAINTAINED") + 1,
                 "STATE-MAINTAINED");
    ...
    case AUTH_REQ_TYPE: {
      ...
      auth_req_type =
          calloc(strlen("AUTHORIZE_AUTHENTICATE") + 1, sizeof(char));

      int auth_req_val =
          start[3] + (start[2] << 8) + (start[1] << 8) + (start[0] << 8);
      if (auth_req_val == 1)
        snprintf(auth_req_type, strlen("AUTHENTICATE_ONLY") + 1,
                 "AUTHENTICATE_ONLY");
      else if (auth_req_val == 2)
        snprintf(auth_sess_st, strlen("AUTHORIZE_ONLY") + 1, "AUTHORIZE_ONLY");
      else if (auth_req_val == 3)
        snprintf(auth_sess_st, strlen("AUTHORIZE_AUTHENTICATE") + 1,
    ...
    case SESS_SRV_FAIL: {
      ...
      sess_fail_type =
          calloc(strlen("TRY_AGAIN_ALLOW_SERVICE") + 1, sizeof(char));

      int sess_fail_val =
          start[3] + (start[2] << 8) + (start[1] << 8) + (start[0] << 8);
      if (sess_fail_val == 0)
        snprintf(sess_fail_type, strlen("REFUSE_SERVICE") + 1,
                 "REFUSE_SERVICE");
      else if (sess_fail_val == 1)
        snprintf(sess_fail_type, strlen("TRY_AGAIN") + 1, "TRY_AGAIN");
      else if (sess_fail_val == 2)
        snprintf(sess_fail_type, strlen("ALLOW_SERVICE") + 1, "ALLOW_SERVICE");
      else if (sess_fail_val == 3)
        snprintf(sess_fail_type, strlen("TRY_AGAIN_ALLOW_SERVICE") + 1,
    ...
    case RES_CODE: {
      ...
      if (res_code >= 1000 && res_code < 2000)
        strncpy(rc_buff, "Informational", strlen("Informational"));
      else if (res_code >= 2000 && res_code < 3000)
        strncpy(rc_buff, "Success", strlen("Success"));
      else if (res_code >= 3000 && res_code < 4000)
        strncpy(rc_buff, "Protocol Errors", strlen("Protocol Errors"));
      else if (res_code >= 4000 && res_code < 5000)
        strncpy(rc_buff, "Transient Failures", strlen("Transient Failures"));
      else if (res_code >= 5000 && res_code < 6000)
        strncpy(rc_buff, "Permanent Failure", strlen("Permanent Failure"));
    ...
    case TIMESTAMP: {
      ...
      time_t tm = start[3] + (start[2] << 8) + (start[1] << 8) +
                  (start[0] << 8); /* CHECK */
