...
flow_callback_proto_init(/* int thread_id, */ pcap_t *p_handle, u_int8_t save) {
  struct flow_callback_proto *flow = malloc(sizeof(struct flow_callback_proto));
  if (!flow)
    perror("fcp malloc failed");
...
/*     pcap_loop(call_thread[thread_id].flow_c->pcap_handle, -1, callback_proto,
 * (u_char*) &thread_id); */
...
static unsigned int
process_packet(const u_char *payload, const u_int16_t size_payload,
               const u_int8_t ip_version, const struct ipv4_hdr *iphv4,
               const struct ipv6_hdr *iphv6, const u_int16_t src_port,
               const u_int16_t dst_port, const u_int8_t proto_id_l3,
               struct flow_callback_proto *fcp, u_int8_t save
  ...
  if (proto_id_l3 == IPPROTO_UDP) {
    ...
    */
    ...
    // dissect
    ret = rtsp_parser(payload, size_payload, json_buffer, JSON_BUFFER_LEN);
    if (ret == -1)
    ...
    */
    ...
    // dissect
    ret = rtcp_parser(payload, size_payload, json_buffer, JSON_BUFFER_LEN);
    if (ret == -1)
  ...
  /* ################# */
    ...
    */
    if (src_port == 443 || dst_port == 443) {
      ...
      */
      ...
      ret = tls_parser(&payload, size_payload, ip_version, flow_key, src_port,
                       dst_port, proto_id_l3, save);
      /* HT_Flows */
      if (ret == -1) {
    ...
    */
    if (src_port == 3868 || dst_port == 3868) {
      ...
      ret =
          diameter_parser(payload, size_payload, json_buffer, JSON_BUFFER_LEN);
      if (ret == -1) {
...
void callback_proto(u_char *args, const struct pcap_pkthdr *pkt_header,
                    const u_char *packet) {
...
/*** Linux Cooked Capture ***/
  ...
  if (is_sctp == 0) {
    ...
    (ip_proto == IPPROTO_TCP) ? (src_port = ntohs(tcp_header->tcp_src_port))
                              : (src_port = ntohs(udp_header->udp_src_port));
    (ip_proto == IPPROTO_TCP) ? (dst_port = ntohs(tcp_header->tcp_dst_port))
                              : (dst_port = ntohs(udp_header->udp_dst_port));
  } else {
  ...
  **/
  check = process_packet(payload, size_payload, ip_version, ipv4_header,
                         ipv6_header, src_port, dst_port, ip_proto, fcp, s
                         /* HT_Flows */);
  if (check == 4) { // TODO FIX
