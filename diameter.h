...
#define DIAMETER_HEADER_JSON                                                   \
  "{\"type\":\"%s\",\"command\":%d,\"app-ID\":%u,\"hop-by-hop-ID\":\"%s\","    \
  "\"end-to-end-ID\":\"%s\","
...
/**
   Application-ID is used to identify for which Diameter application the message
is belong to. The application can be an authentication application, an
...
struct avp_header_t {
  ...
  u_int8_t flag;
  u_int8_t
      length[3]; /* Values not multiple of four-octets is followed by padding to
                    have 32-bit boundary for the next AVP (if exists) */
};
...
**/
// Parse packet, check if it's Diameter and create JSON buffer with protocol
// information
