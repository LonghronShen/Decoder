...
static void save_certificate_FILE(const unsigned char *cert,
                                  u_int16_t cert_len) {
  FILE *fw;
  X509 *x_cert;
  ...
  strftime(filename, sizeof(filename),
           "certificates/cert_%Y-%m-%d_%H-%M-%S-%%03u.der", timeinfo);
  ...
  // function to convert raw data (DER) to PEM certificate (good for parsing
  // with openssl)
...
/* static struct Certificate split_Server_Certificate(char * certificate,
 * u_int16_t cert_len) */
/* {  */
/* } */
...
// ADD CLI ID
static void add_cli_id(struct Hash_Table **flow_in,
                       struct Handshake **handshake, u_int8_t len_id) {
  // copy cli_rand
...
// ADD SRV ID
static void add_srv_id(struct Hash_Table **flow_in,
                       struct Handshake **handshake, u_int8_t len_id) {
  // copy srv_rand
...
static void update_cert(struct Hash_Table **flow_in,
                        struct Handshake **handshake, u_int8_t len_cert,
                        u_int8_t cc) {
  // copy certificate_S
  if (len_cert > 1) {
    if (cc == CERT_S) {
      (*flow_in)->handshake->certificate_S =
          malloc(sizeof(unsigned char) * len_cert);
      memcpy((*flow_in)->handshake->certificate_S, (*handshake)->certificate_S,
             len_cert);
    } else if (len_cert == CERT_C) {
      (*flow_in)->handshake->certificate_C =
          malloc(sizeof(unsigned char) * len_cert);
      memcpy((*flow_in)->handshake->certificate_C, (*handshake)->certificate_C,
             len_cert);
    }
  } else
    (*flow_in)->handshake->sessID_s = NULL;
...
// ADD FLOW
static void add_flow(/* struct Hash_Table * HT_Flows,  */ struct Flow_key *key,
                     struct Handshake *handshake, u_int8_t flag,
                     u_int8_t len_id) {
...
int tls_parser(const u_char **payload, const u_int16_t size_payload,
  ...
  */
  if (proto_id_l3 == IPPROTO_TCP && ((src_port == 443 || dst_port == 443) ||
    ...
    struct header_tls_record *hdr_tls_rec =
        (struct header_tls_record *)(*payload);
    ...
    if (type == HANDSHAKE) {
      ...
      do {
        ...
        case CLIENT_HELLO: {
          ...
          offset = TLS_HEADER_LEN + HANDSK_HEADER_LEN + 2 + RANDOM + len_id +
                   2 + cipher_len;
          pp = pp + 2 + cipher_len;
          ...
          if (offset < size_payload) {
            ...
            if (offset < size_payload) {
              ...
              if (offset < size_payload) {
                /**
                   More extensions
                   Note: u_int to avoid possible overflow on extension_len
                   addition
                */
                u_int exts_offset = 1;
        ...
        case SERVER_HELLO: {
          ...
          if (pp[0] != 0x00 &&
              (pp[1] != 0x9d || pp[1] != 0x9c || pp[1] != 0x3d ||
               pp[1] != 0x3c || pp[1] != 0x35 || pp[1] != 0x2f ||
               pp[1] != 0xff)) {
            fprintf(stderr, "Invalid Chipher Suite. No DHE/EDH availlable for "
                            "decription\n");
            return -1;
          }
        ...
        case CERTIFICATE: {
          u_int16_t hh_len = hand_hdr->len[2] + (hand_hdr->len[1] << 8) +
                             (hand_hdr->len[0] << 8);
          ...
          if (cert_len_total > 0) {
            ...
            do { // more than one certificate
              ...
              ***/
              // handshake.certificate_S = split_Server_Certificate(cert,
              // cert_len);
              pp = pp + 3 + subcert_len;
          ...
          if (offset < size_payload) {
            if (cert_len_total > 0) {
              if (pp[5] != 0x0c && pp[5] != 0x16 && pp[5] != 0x10) {
                ...
                // search flow and eventually inser new in HT update old
                add_flow(/* HT_Flows,  */ flow_key, handshake, CERT_S,
                         cert_len_total);
            }
            more_records = 0;
            break;
...
#ifdef ECDH
          struct server_key_exch_tls_12_ECDH s_key_exc =
              (struct server_key_exch_tls_12_ECDH)pp;
#endif
#ifdef DH
          struct server_key_exch_tls_12_DH s_key_exc =
              (struct server_key_exch_tls_12_DH)pp;
#endif
          int hand_hdr_len = (hand_hdr->len[2]) + (hand_hdr->len[1] << 8) +
                             (hand_hdr->len[0] << 8);
          ...
          if (hand_hdr_len > 33) {
            ...
            */
            if (ntohs(hdr_tls_rec->version) == TLS1) {
              ...
              handshake->enc_pre_master_secret =
                  malloc(sizeof(u_int8_t) * pms_len);
              // save the pre-master secret (encrypted)
              memcpy(handshake->enc_pre_master_secret, pp + 2, pms_len);

              /** DECRYPT PRE-MASTER SECRET USING SERVER PUB KEY FROM
               * CERTIFICATE **/
        ...
        case CERTIFICATE_REQUEST: {
          ...
          int hand_hdr_len = (hand_hdr->len[2]) + (hand_hdr->len[1] << 8) +
                             (hand_hdr->len[0] << 8);
          pp = pp + hand_hdr_len;
          offset = TLS_HEADER_LEN + HANDSK_HEADER_LEN + sizeof(cert_req) +
                   cert_req->dist_name_len;
        ...
        case FINISHED: {
          ...
          // set handshake fin to TRUE
          HASH_FIND(hh, HT_Flows, &flow_key, sizeof(struct Flow_key), old);
          if (old) {
            old->is_handsk_fin = TRUE;
            HASH_REPLACE(hh, HT_Flows, flow_key_hash, sizeof(struct Flow_key),
                         old, el);
          }
          more_records = 1;
          break;
